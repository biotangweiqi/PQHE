...
# function: estimate heritability and its confidence limits 
# usage: Rscript this_Script.R case.conf.txt
...
if(is.null(conf$b)) cat("Error:\n");
if(is.null(conf$m1)) cat("Error:\n");
if(is.null(conf$n)) cat("Error:\n");
if(is.null(conf$AF1)) cat("Error:\n");
...
af2 <- NA;# allele frequency of pool 1, not required
...
if(!is.null(conf$AF2)) af2 <- as.numeric(conf$AF2);# allele frequency of pool 1, not required
...
# list of explanation
h2_explain <- list(h2B = "Broad (total) heratibility",
...
# estimations for null distribution
nh2A <- c();

# estimating for null distribution
for(i in 1:Times){
  #
  afu.data <- data.frame(afu = c(uaf[i], u2af[i]), pr = c(pL, pH));
  afu.data <- afu.data[order(c(uaf[i], u2af[i]), decreasing = TRUE),];
  #cat(afu.data$afu[1], "\t", afu.data$afu[2], "\n");
  if(afu.data$afu[1] <= 0.5 | afu.data$afu[2] >= 0.5){ 
    nh2A <- c(nh2A, NA);
    next
  }
  
  # estimating by different modes
  if(b < 0.5)  nhh <- estimate_by_reduced_mode(b, afu.data$afu[1], afu.data$afu[2], afu.data$pr[1], afu.data$pr[2]);
  if(b == 0.5) nhh <- estimate_by_adjusted_reduced_mode(b, afu.data$afu[1], afu.data$afu[2], taf[i], afu.data$pr[1], afu.data$pr[2]);
  # save
  if(nhh$precis > 0.001 ){
    #precis_mk <- "YES";
    nhh$h2A <- NA;
  }
  if(is.null(nhh$h2A) ) nhh$h2A <- NA;
  nh2A <- c(nh2A, nhh$h2A);
}
...
# display results
cat("overview of heritability estimation for null distribution:\n");
if(length( which( is.na(nh2A) ) ) < Times ){ 
  cat("Additive effect heritability.\n")
  display_estimation(nh2A);
}

# write out
out <- data.frame(taf, haf, laf, sprintf("%.4f", haf - laf), uaf, u2af, sprintf("%.4f", abs(uaf - u2af)), fh2B, fh2N, fh2A, fh2D, nh2A);
colnames(out) <- c("Total AF", "fQH", "fQL", "fQH - fQL", "fQU1", "fQU2", "abs fQU1 - fQU2", "h2B", "h2N", "h2A", "h2D", "null_h2A");

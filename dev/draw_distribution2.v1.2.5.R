...
# title and labels
#p <- p + ggtitle("Mixture distribution") + theme(plot.title = element_text(hjust = 0.5, size = rel(2)) )
...
p <- p + theme(axis.title = element_text(size = rel(1.8) ), axis.text = element_text(size = rel(1.5)) )
...
p <- p + annotate("text", x = min(dat$x)+0.1, y = max(dat$fx)*0.50, hjust = 0, vjust = 1, label = eqn_b, parse = TRUE, size = 7 )
eqn_a0 <- as.character( as.expression(substitute(italic("a")[0]==a0, list(a0 = a0) ) ) )
p <- p + annotate("text", x = min(dat$x)+0.1, y = max(dat$fx)*0.40, hjust = 0, vjust = 1, label = eqn_a0, parse = TRUE, size = 7 )
eqn_d0 <- as.character( as.expression(substitute(italic("d")[0]==d0, list(d0 = d0) ) ) )
p <- p + annotate("text", x = min(dat$x)+0.1, y = max(dat$fx)*0.30, hjust = 0, vjust = 1, label = eqn_d0, parse = TRUE, size = 7 )

# legend
lab.y3 <- expression(paste("(3) ", pi[QQ], italic(f)[QQ], "(",italic(x),")=", italic(b), italic(phi), "(", italic(x), "-", italic(a)[0], ")", sep = "") )
lab.y2 <- expression(paste("(2) ", pi[Qq], italic(f)[Qq], "(",italic(x),")=(", "1-2", italic(b), ")", italic(phi), "(", italic(x), "-", italic(d)[0], ")", sep = "") )
lab.y1 <- expression(paste("(1) ", pi[qq], italic(f)[qq], "(",italic(x),")=", italic(b), italic(phi), "(", italic(x), "+", italic(a)[0], ")", sep = "") )
...
p <- p + scale_linetype_manual(
  name = NULL,
  limits = c("fx", "y1", "y2", "y3"),
  breaks = c("fx", "y1", "y2", "y3"), 
  labels = c(lab.fx, lab.y1, lab.y2, lab.y3),
  values = c(1, 2, 2, 2)
)

#p <- p + annotate("text", x = dat$x[which.max(dat$fx)], y = max(dat$fx), label = "(1)", hjust = 0, vjust = 0.5)
p <- p + annotate("text", x = dat$x[which.max(dat$y1)], y = max(dat$y1), label = "(1)", hjust = 0.5, vjust = -0.5, size = 6)
p <- p + annotate("text", x = dat$x[which.max(dat$y2)], y = max(dat$y2), label = "(2)", hjust = 0.5, vjust = -0.5, size = 6)
p <- p + annotate("text", x = dat$x[which.max(dat$y3)], y = max(dat$y3), label = "(3)", hjust = 0.5, vjust = -0.5, size = 6)
...
#p <- p + scale_fill_discrete(name = "Parameters:", breaks = c("bb", "aa", "dd"), labels = c(eqn_b, eqn_a0, eqn_d0) )
#p <- p + scale_fill_discrete(name = NULL, breaks = c("bb", "aa", "dd"), labels = c(eqn_b, eqn_a0, eqn_d0) )

# theme of legend
p <- p + theme(legend.position = c(0.05,0.98), legend.justification = c(0,1) )
#p <- p + theme(legend.background = element_rect(fill = "white", color = "black") )
p <- p + theme(legend.background = element_blank() )
p <- p + theme(legend.text = element_text(size = rel(1.8) ), legend.title = element_text(size = rel(1.5) ) )
p <- p + theme(legend.key.height = unit(1.5, "cm"), legend.text.align = 0)
#p <- p + theme(legend.key = element_rect(linetype = NA, inherit.blank = TRUE) )
...
p <- p + annotate("text", x = xL, y = yL, hjust = 1.1, vjust = 0, label = eqn_xL, parse = TRUE, size = 7 )
eqn_xH <- as.character( as.expression(substitute(italic("x")["H"]==xH, list(xH = sprintf("%.3f",xH) ) ) ) )
p <- p + annotate("text", x = xH, y = yH, hjust = -0.1, vjust = 0, label = eqn_xH, parse = TRUE, size = 7 )
# draw arrow
...
p <- p + annotate("text", x = xpL-0.05, y = ypL+0.008, hjust = 1, vjust = 0, label = eqn_pL, parse = TRUE, size = 7 )
p <- p + annotate("text", x = xpH+0.05, y = ypH+0.008, hjust = 0, vjust = 0, label = eqn_pH, parse = TRUE, size = 7 )
...
# save
file_tiff <- paste("mixture_distribution3_b_is_", b, "_a0_is_", a0, "_d0_is_", d0, ".tiff", sep = "")
ggsave(file_tiff, width = 12, height = 6.6, device = "tiff", dpi = 1200, compression = "lzw")

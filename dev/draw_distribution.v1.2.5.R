...
p <- ggplot(data = melt_data, aes(x, y) ) + geom_line(size = 1.2, linetype = "solid", aes(color = distribution))
# title and labels
...
p <- p + annotate("text", x = min(dat$x)+0.3, y = max(dat$fx)*0.60, hjust = 0, vjust = 1, label = eqn_b, parse = TRUE, size = 6.3 )
eqn_a0 <- as.character( as.expression(substitute(italic("a")[0]==a0, list(a0 = a0) ) ) )
p <- p + annotate("text", x = min(dat$x)+0.3, y = max(dat$fx)*0.53, hjust = 0, vjust = 1, label = eqn_a0, parse = TRUE, size = 6.3 )
eqn_d0 <- as.character( as.expression(substitute(italic("d")[0]==d0, list(d0 = d0) ) ) )
p <- p + annotate("text", x = min(dat$x)+0.3, y = max(dat$fx)*0.46, hjust = 0, vjust = 1, label = eqn_d0, parse = TRUE, size = 6.3 )
...
ggsave(file_tiff, width = 12, height = 8, device = "tiff", dpi = 900, compression = "lzw")
